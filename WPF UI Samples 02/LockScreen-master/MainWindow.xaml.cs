...
namespace LockScreen
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private void BtnLogin_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                Message("Success", "Logging In", NotificationType.Success);
                System.Windows.Application.Current.Shutdown();
            }
            else
            {
                Message("Error", "The password is invalid", NotificationType.Error);
            }
        }
            ...
            try
            {
                if (!string.IsNullOrEmpty(myPasswordBox.Password) && myPasswordBox.Password == myPasswordBoxRepeat.Password)
                {
                    ...
                    if (liteDb.Update(setting))
                    {
                        ...
                        Message("Success", "Information saved", NotificationType.Information);
                        show_hideSettingBox();
                    }
                    else
                    {
                        Message("Program error", "Something went wrong, please try again", NotificationType.Information);
                    }
                }
                else
                {
                    Message("Warning", "Passwords must be the same and cannot be empty", NotificationType.Warning);
                }
            }
        ...
        private void ButtonSetting_Click(object sender, RoutedEventArgs e)
        {
            if (FloatingPasswordBox.Password == liteDb.Select(1).passWord)
            {
                Message("Success", "Welcome to Settings", NotificationType.Success);
                show_hideSettingBox();
            }
            else
            {
